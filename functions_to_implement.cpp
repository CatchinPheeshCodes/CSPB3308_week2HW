...
{
	//find first occurance of s2 in s1, if it exists.
	size_t pos = s1.find(s2);

	//if s2 exists in s1, erase chars in range of first position of s2 to last position of s2.
...
{
	if(num == 0) 
		throw std::runtime_error("Number is 0."); 
	else if(num < 0) 
		return -1;
	else
		return 1;
